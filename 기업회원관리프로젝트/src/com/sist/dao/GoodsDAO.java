...
public class GoodsDAO {
	...
	public List<GoodsVO> goodsListData(int type, int page) {
		...
		try {
			...
			getConnection();
			// 2. SQL문장
			String sql = "SELECT no,goods_poster,goods_name,goods_price " + "FROM " + tables[type] + " ORDER BY no ASC "
			...
			// previous() => 마지막 => 처음
			/*
			 * 데이터 출력 getInt() / getString() / getDouble() getDate()
			 */
	...
	/*
	 * 1. List => 조건없는 경우 / 페이지 나누기 2. VO => SELECT 컬럼,컬럼.... 3. 컬럼이 한개 => 해당 데이터형
	 * ----------------------------------- => pwd : 로그인 String
	 */
	/*
	 * NO NOT NULL NUMBER GOODS_NAME NOT NULL VARCHAR2(1000) GOODS_SUB
	 * VARCHAR2(1000) GOODS_PRICE NOT NULL VARCHAR2(50) GOODS_DISCOUNT NUMBER
	 * GOODS_FIRST_PRICE VARCHAR2(20) GOODS_DELIVERY NOT NULL VARCHAR2(20)
	 * GOODS_POSTER VARCHAR2(260) HIT NUMBER 1. 목록 (row가 여러개) | VO => 여러개 저장 (List)
	 * 2. 상세보기 => row (1개) --------- VO한개 SELECT no FROM goods=> int List<Integer>
	 * 컬럼이 두개이상 => VO
	 * 
	 */
	...
	// 구매
	/*
	 * NO NOT NULL NUMBER ID VARCHAR2(20) TYPE NOT NULL NUMBER GNO NOT NULL NUMBER
	 * ACCOUNT NUMBER PRICE NUMBER REGDATE
	 */
	...
	// 조인
	/*
	 * 고객 <구매> 도서 | id , 도서번호 aaa 1 1 = INNER (EQUI_JOIN) => INTERSECT (교집합) = OUTER
	 * => LEFT/RIGHT => INTERSECT + MINUS
	 */
	public List<BuyVO> buyListData(String id) {
		...
		try {
			...
			String sql = "SELECT b.no,goods_poster,goods_name,account," + "TO_CHAR(regdate,'YYYY-MM-DD'),price "
					+ "FROM buy b JOIN goods_all g " + "ON b.gno=g.no " + "AND b.id=? " + "ORDER BY regdate DESC";
			// 본인만
			ps = conn.prepareStatement(sql);
			// ?에 값을 채운다
			ps.setString(1, id);
